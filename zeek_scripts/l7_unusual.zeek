...
# A script for checking for some common application layer protocols over unusual IP layer protocols
# check for http over unusual protocols
global http_counter: count = 0;
# check for ftp over unusual protocols
global ftp_counter: count = 0;
# check for tcp over unusual protocols
global tcp_counter: count = 0;
# current IP layer protocol
global current_protocol: count = 0;
...
export {
    redef enum Log::ID += { LOG, LOG2, LOG_NEW, LOG_UNUSUAL };
    ...
    type known_unusual: record {
	ts: time        &log;
	application: string &log;
        ip_protocol: count &log;
        ip_protocol_name: string &log;
    };
}
...
event zeek_init() &priority=5
    ...
    # Create a stream for logging common applications over unusual protocols
    Log::create_stream(Unusual_protocols::LOG_UNUSUAL, [$columns=known_unusual, $path="over_unusual"]);
...
event ftp_reply(c: connection, code: count, msg: string, cont_resp: bool)
	...
	# check if the IP protocol is something other than TCP
		...
		if(current_protocol < 144)
        	{
                	proto_name = protocol_names[current_protocol];
        	}

		local rec: Unusual_protocols::known_unusual = [$ts=network_time(), $application="FTP", $ip_protocol=current_protocol, $ip_protocol_name=proto_name];
     		Log::write(Unusual_protocols::LOG_UNUSUAL, rec);
	}
    }
...
event http_all_headers(c: connection, is_orig: bool, hlist: mime_header_list)
	...
	if (current_protocol != 6)
	{
		local proto_name: string = "N/A";
                ...
                local rec: Unusual_protocols::known_unusual = [$ts=network_time(), $application="HTTP", $ip_protocol=current_protocol, $ip_protocol_name=proto_name];
...
event ssh_capabilities(c: connection, cookie: string, capabilities: SSH::Capabilities)
	...
	# check if the IP protocol is something other than TCP
		...
		local proto_name: string = "N/A";
                ...
                local rec: Unusual_protocols::known_unusual = [$ts=network_time(), $application="SSH", $ip_protocol=current_protocol, $ip_protocol_name=proto_name];
