	...
	EventHandlerPtr f1 = nullptr;
		...
		//if ( packet->ip_hdr->LastHeader() == IPPROTO_ESP )
		{
			...
			if(protocol == 4)
			{
				...
				snprintf(esp_string, sizeof(esp_string), "%u", data[24]);
                        	esp_protocol = atoi(esp_string);
			}
			else if (data[6] == 50 && protocol == 6)
			{
				//reporter->Info("ipv6 working");
                                snprintf(esp_string, sizeof(esp_string), "%u", data[44]);
                                esp_protocol = atoi(esp_string);
			}
		}

                mgr.Enqueue(f1, make_intrusive<AddrVal>(packet->ip_hdr->SrcAddr()), make_intrusive<AddrVal>(packet->ip_hdr->DstAddr()), val_mgr->Count(packet->proto), val_mgr->Count(esp_protocol));//packet->Weird("Found Protocol - TCP");
