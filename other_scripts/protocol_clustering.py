...
log_file.writelines("time protocol_count")
log_file.writelines("\n")

for line in Lines: 
    split_line = line.split()
    if split_line[1] == 'proto_count' and int(split_line[2]) == protocol_number:
        log_file.writelines(split_line[0])
        log_file.writelines(" ")
        log_file.writelines(split_line[4])
        log_file.writelines("\n")
log_file.close()
...
timeAndCount = processed_csv[["time", "protocol_count"]]
